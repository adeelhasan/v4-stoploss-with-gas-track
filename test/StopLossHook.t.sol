...
contract StopLossHookTest is HookTest, Deployers {
    ...
    function testOrderExecuteZeroForOneSinglePosition() public {
        int24 tick = -300;
        ...
        //console2.log("Tokens against position: ", stopLossHook.stopLossPositions(poolId, tick, zeroForOne));
        assertEq(positionAmount, stopLossHook.balanceOf(user1, tokenIdAtPosition));
        assertEq(stopLossHook.gasBalances(user1), 1 ether);
        vm.stopPrank();
        ...
        uint256 user1GasBalanceBefore = stopLossHook.gasBalances(user1);
        uint256 user2GasBalanceBefore = stopLossHook.gasBalances(user2);

        //pass the swap executor address into hookData so that it is available deep downstream
        ...
        uint256 user1GasBalanceAfter = stopLossHook.gasBalances(user1);
        uint256 user2GasBalanceAfter = stopLossHook.gasBalances(user2);

        //user1 should have lost some balance to pay for the stopLoss swap-execution
